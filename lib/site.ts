export const SITE_NAME = "StableMoney.Dev"
/** Production origin — override with NEXT_PUBLIC_SITE_URL in local dev if needed */
export const SITE_CANONICAL_URL = "https://stablemoney.dev"
export const SITE_TAGLINE =
  "Technical reference for engineers building with stablecoins"
export const SITE_DESCRIPTION =
  "Engineering-focused stablecoin reference: networks, contracts, features, and risks for top stablecoins by market cap."

/** Bump when static data meaningfully changes */
export const LAST_UPDATED = "2026-03-21"

/** Display-only aggregate for the stats bar (static, not live) */
export const TOTAL_MARKET_CAP_DISPLAY = "~$312B"
